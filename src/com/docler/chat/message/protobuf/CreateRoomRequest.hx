...
class CreateRoomRequest extends protohx.Message {

	/** LABEL_REQUIRED host : TYPE_STRING = 1 */
	@:isVar public var host(get, set):PT_String;
	public inline function setHost(value:PT_String):CreateRoomRequest{
		this.host = value;
		return this;
	}
	...
	public inline function setRoomName(value:PT_String):CreateRoomRequest{
		this.roomName = value;
		return this;
	}
	...
	public inline function setMessageId(value:PT_String):CreateRoomRequest{
	...
	override public function readFromSlice(input:PT_InputStream, bytesAfterSlice:PT_UInt):Void {
		...
		while (hasBytes(input, bytesAfterSlice)) {
			...
			case 1:
				if (host__count != 0) {
					throw new PT_IOError('Bad data format: CreateRoomRequest.host cannot be set twice.');
				}
				++host__count;
				this.host = protohx.ReadUtils.read__TYPE_STRING(input);
			case 2:
				if (room_name__count != 0) {
					throw new PT_IOError('Bad data format: CreateRoomRequest.roomName cannot be set twice.');
				}
				++room_name__count;
				this.roomName = protohx.ReadUtils.read__TYPE_STRING(input);
			case 3:
				if (message_id__count != 0) {
					throw new PT_IOError('Bad data format: CreateRoomRequest.messageId cannot be set twice.');
				}
				++message_id__count;

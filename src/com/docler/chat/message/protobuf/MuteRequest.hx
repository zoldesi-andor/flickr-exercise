...
class MuteRequest extends protohx.Message {

	/** LABEL_REQUIRED host : TYPE_STRING = 1 */
	@:isVar public var host(get, set):PT_String;
	public inline function setHost(value:PT_String):MuteRequest{
		this.host = value;
		return this;
	}
	...
	public inline function setRoomName(value:PT_String):MuteRequest{
		this.roomName = value;
		return this;
	}
	...
	public inline function setUserType(value:PT_String):MuteRequest{
		this.userType = value;
		return this;
	}
	...
	public inline function setMessageId(value:PT_String):MuteRequest{
	...
	override public function readFromSlice(input:PT_InputStream, bytesAfterSlice:PT_UInt):Void {
		...
		while (hasBytes(input, bytesAfterSlice)) {
			...
			case 1:
				if (host__count != 0) {
					throw new PT_IOError('Bad data format: MuteRequest.host cannot be set twice.');
				}
				++host__count;
				this.host = protohx.ReadUtils.read__TYPE_STRING(input);
			case 2:
				if (room_name__count != 0) {
					throw new PT_IOError('Bad data format: MuteRequest.roomName cannot be set twice.');
				}
				++room_name__count;
				this.roomName = protohx.ReadUtils.read__TYPE_STRING(input);
			case 3:
				if (user_type__count != 0) {
					throw new PT_IOError('Bad data format: MuteRequest.userType cannot be set twice.');
				}
				++user_type__count;
				this.userType = protohx.ReadUtils.read__TYPE_STRING(input);
			case 4:
				if (message_id__count != 0) {
					throw new PT_IOError('Bad data format: MuteRequest.messageId cannot be set twice.');
				}
				++message_id__count;

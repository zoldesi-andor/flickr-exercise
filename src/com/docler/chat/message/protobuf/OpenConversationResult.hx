...
class OpenConversationResult extends protohx.Message {

	/** LABEL_REQUIRED result : TYPE_MESSAGE = 1 */
	@:isVar public var result(get, set):com.docler.chat.message.protobuf.ResultBase;
	public inline function setResult(value:com.docler.chat.message.protobuf.ResultBase):OpenConversationResult{
		this.result = value;
		return this;
	}
	...
	/** LABEL_OPTIONAL conversation_name : TYPE_STRING = 2 */
	@:isVar public var conversationName(get, set):PT_String;
	public inline function setConversationName(value:PT_String):OpenConversationResult{
		this.conversationName = value;
		return this;
	}

	public function clearConversationName():Void {
		this.conversationName = null;
	}

	inline public function hasConversationName():Bool {
	...
	override public function readFromSlice(input:PT_InputStream, bytesAfterSlice:PT_UInt):Void {
		...
		while (hasBytes(input, bytesAfterSlice)) {
			...
			case 1:
				if (result__count != 0) {
					throw new PT_IOError('Bad data format: OpenConversationResult.result cannot be set twice.');
				}
				++result__count;
				this.result = new com.docler.chat.message.protobuf.ResultBase();
			...
			case 2:
				if (conversation_name__count != 0) {
					throw new PT_IOError('Bad data format: OpenConversationResult.conversationName cannot be set twice.');
				}

...
class ReceiveRoomDeleted extends protohx.Message {

	/** LABEL_REQUIRED room_name : TYPE_STRING = 1 */
	@:isVar public var roomName(get, set):PT_String;
	public inline function setRoomName(value:PT_String):ReceiveRoomDeleted{
		this.roomName = value;
		return this;
	}
		...
		while (hasBytes(input, bytesAfterSlice)) {
			...
			case 1:
				if (room_name__count != 0) {
					throw new PT_IOError('Bad data format: ReceiveRoomDeleted.roomName cannot be set twice.');
				}
				++room_name__count;

...
class UnmuteRequest extends protohx.Message {

	/** LABEL_REQUIRED host : TYPE_STRING = 1 */
	@:isVar public var host(get, set):PT_String;
	public inline function setHost(value:PT_String):UnmuteRequest{
		this.host = value;
		return this;
	}
	...
	public inline function setRoomName(value:PT_String):UnmuteRequest{
		this.roomName = value;
		return this;
	}
	...
	/** LABEL_REQUIRED user_type : TYPE_STRING = 3 */
	@:isVar public var userType(get, set):PT_String;
	public inline function setUserType(value:PT_String):UnmuteRequest{
		this.userType = value;
		return this;
	}

	public function set_userType(value:PT_String):PT_String{
		return this.userType = value;
	}

	public function get_userType():PT_String {
	...
	public inline function setMessageId(value:PT_String):UnmuteRequest{
	...
	override public function writeToBuffer(output:PT_OutputStream):Void {
		...
		protohx.WriteUtils.write__TYPE_STRING(output, this.userType);
	...
	override public function readFromSlice(input:PT_InputStream, bytesAfterSlice:PT_UInt):Void {
		...
		var user_type__count:PT_UInt = 0;
		...
		while (hasBytes(input, bytesAfterSlice)) {
			...
			case 1:
				if (host__count != 0) {
					throw new PT_IOError('Bad data format: UnmuteRequest.host cannot be set twice.');
				}
				++host__count;
				this.host = protohx.ReadUtils.read__TYPE_STRING(input);
			case 2:
				if (room_name__count != 0) {
					throw new PT_IOError('Bad data format: UnmuteRequest.roomName cannot be set twice.');
				}
				++room_name__count;
				this.roomName = protohx.ReadUtils.read__TYPE_STRING(input);
			case 3:
				if (user_type__count != 0) {
					throw new PT_IOError('Bad data format: UnmuteRequest.userType cannot be set twice.');
				}
				++user_type__count;
				this.userType = protohx.ReadUtils.read__TYPE_STRING(input);
			case 4:
				if (message_id__count != 0) {
					throw new PT_IOError('Bad data format: UnmuteRequest.messageId cannot be set twice.');
				}
				++message_id__count;

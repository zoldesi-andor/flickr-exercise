...
class UnsubscribeRoomResult extends protohx.Message {

	/** LABEL_REQUIRED result : TYPE_MESSAGE = 1 */
	@:isVar public var result(get, set):com.docler.chat.message.protobuf.ResultBase;
	public inline function setResult(value:com.docler.chat.message.protobuf.ResultBase):UnsubscribeRoomResult{
		this.result = value;
		return this;
	}
	...
	override public function readFromSlice(input:PT_InputStream, bytesAfterSlice:PT_UInt):Void {
		...
		while (hasBytes(input, bytesAfterSlice)) {
			...
			case 1:
				if (result__count != 0) {
					throw new PT_IOError('Bad data format: UnsubscribeRoomResult.result cannot be set twice.');
				}
				++result__count;
				this.result = new com.docler.chat.message.protobuf.ResultBase();
